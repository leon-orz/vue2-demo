  ...
  <div class="secondary-page-wrap">
    ...
    <div class="filter-wrap has-return">
      <el-form ref="form" :inline="true">
        <el-form-item>
          <el-select style="width: 150px" v-model="form.factory" placeholder="请选择厂区" clearable @change="changeFactory()">
            <el-option v-for="item in factoryList" :key="item.code" :label="item.desc" :value="item.code"> </el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
          <el-select style="width: 150px" v-model="form.areaCode" placeholder="请选择区域" clearable @change="changeArea()">
        ...
        <el-form-item>
          <el-select style="width: 150px" v-model="form.device" placeholder="设备" clearable @change="changeDevice()">
            <el-option v-for="item in deviceList" :key="item.id" :label="item.equipmentName" :value="item.equipmentCode">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
          <el-select style="width: 150px" v-model="form.subCode" placeholder="子设备" clearable @change="changeSubDevice()">
            <el-option v-for="item in subDeviceList" :key="item.code" :label="item.name" :value="item.code"> </el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
export default {
  ...
  created() {
    ...
    this.form.subCode = this.routeData.subCode || ''
  ...
  methods: {
    ...
    // 查询设备信息
    queryDeviceInfo() {
      const cur = this.subDeviceList.find((item) => item.equipmentCode == this.form.subCode)
      this.$api.equipment
        .SonEquipmentQuerySonEquipmentList({
          startTime: this.timeR && this.timeR[0] ? this.timeR[0] : '',
          endTime: this.timeR && this.timeR[1] ? this.timeR[1] : '',
          equipmentCode: cur ? cur.equipmentCode : this.form.subCode
        })
        .then((res) => {
          if (res) {
    ...
    querySubDevice() {
      ...
      this.$api.equipment
        ...
        .then((res) => {
          if (res && res.list) {
            this.subDeviceList = res.list || []
            if (!this.form.subCode) {
              this.form.subCode = this.subDeviceList[0].equipmentCode
            }
          } else {
            this.subDeviceList = []
          }
    ...
    queryDeviceByCode() {
      const cur = this.subDeviceList.find((item) => item.equipmentCode == this.form.subCode)
      this.$api.equipment
        .QuerySonEquipmentListByEquipmentCode({
          code: cur ? cur.equipmentCode : this.form.subCode,
          type: this.form.deviceType
        })
        .then((res) => {
          if (res) {
    ...
    queryThroughputLog() {
      const cur = this.subDeviceList.find((item) => item.equipmentCode == this.form.subCode)
      this.$api.equipment
        .QuerySonPagePathRegisterMainList({
          equipmentCode: cur ? cur.equipmentCode : this.form.subCode,
          startTime: this.timeR && this.timeR[0] ? this.timeR[0] : '',
          endTime: this.timeR && this.timeR[1] ? this.timeR[1] : '',
          ...this.tuntuPage
    ...
    queryWarehousing() {
      this.$api.equipment
        .QuerySonEquipmentThroughputStorageProportionData({
          type: this.form.deviceType,
          equipmentCode: this.form.subCode
        })
        .then((res) => {
          if (res) {
    ...
    queryUtilization() {
      this.$api.equipment
        .QuerySonEquipmentThroughputTimeStatusData({
          equipmentCode: this.form.subCode,
          startTime: this.timeR && this.timeR[0] ? this.timeR[0] : '',
          endTime: this.timeR && this.timeR[1] ? this.timeR[1] : ''
        })
    ...
    queryUtilizationBar() {
      this.$api.equipment
        .QuerySonEquipmentThroughputStatusStatistics({
          equipmentCode: this.form.subCode,
    ...
    queryNum() {
      this.$api.equipment
        .QuerySonEquipmentThroughputTaskData({
          equipmentCode: this.form.subCode,
          startTime: this.timeR && this.timeR[0] ? this.timeR[0] : '',
          endTime: this.timeR && this.timeR[1] ? this.timeR[1] : ''
        })
    ...
    queryNumStatistics() {
      this.$api.equipment
        .QuerySonHandlingCapacityStatistics({
          equipmentCode: this.form.subCode,
          time: this.typeData.find((item) => item.code == this.activeName).desc,
          startTime: this.timeR && this.timeR[0] ? this.timeR[0] : '',
          endTime: this.timeR && this.timeR[1] ? this.timeR[1] : ''
